...
class AlertController {
    ...
    func checkForAlert() {
        ...
        alertReturned = false
        Database.database().reference().child("BannerAlert/message").observe(.value) { (snapshot) in
            if let alertMessage = snapshot.value as? String, alertMessage != "nil", alertMessage != "null", alertMessage != "" {
